...
MoonTV 支持完整的用户权限管理体系，包括三种用户角色：

- **站长 (Owner)**：拥有所有权限，通过环境变量配置
- **管理员 (Admin)**：可以管理普通用户和系统配置
- **普通用户 (User)**：只能观看内容，无法访问管理后台

**注意**：用户管理功能需要使用 Redis/Kvrocks/Upstash 存储，不支持 localstorage 模式。

详细的用户管理指南请参考 [用户管理指南](project-document/用户管理指南.md)。
